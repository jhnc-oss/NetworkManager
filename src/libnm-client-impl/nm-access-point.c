...
nm_access_point_connection_valid(NMAccessPoint *ap, NMConnection *connection)
{
    ...
    if (ap_freq) {
        ...
        if (nm_streq0(setting_band, "6GHz")) {
            if (ap_freq < NM_UTILS_MIN_6GHZ || ap_freq > NM_UTILS_MAX_6GHZ)
                return FALSE;
        } else if (nm_streq0(setting_band, "5GHz") || nm_streq0(setting_band, "a")) {
            if (ap_freq < NM_UTILS_MIN_5GHZ || ap_freq > NM_UTILS_MAX_5GHZ)
                return FALSE;
        } else if (nm_streq0(setting_band, "2.4GHz") || nm_streq0(setting_band, "bg")) {
            if (ap_freq < NM_UTILS_MIN_2GHZ || ap_freq > NM_UTILS_MAX_2GHZ)
                return FALSE;
        }
