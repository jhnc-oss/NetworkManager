                             ...
                             guint8      *out_num_channels)
{
    ...
    if (!combs || combs->n_combinations == 0)
        return FALSE;

    /* Check each combination to find a valid allocation for both interface types */
    for (i = 0; i < combs->n_combinations; i++) {
        NMWifiIfaceCombination *comb = &combs->combinations[i];
         ...
         */
        for (j = 0; j < comb->n_limits; j++) {
            NMWifiIfaceCombLimit *limit_a = &comb->limits[j];

            /* Check if limit_a supports iftype1 */
            if (!(limit_a->types & (1 << iftype1)))
                continue;

            for (k = 0; k < comb->n_limits; k++) {
                NMWifiIfaceCombLimit *limit_b = &comb->limits[k];
