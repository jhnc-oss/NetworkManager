...
_parse_lnk_hsr(const char *kind, struct nlattr *info_data)
{
    ...
    props        = &obj->lnk_hsr;
    props->port1 = nla_get_u32(tb[IFLA_HSR_PORT1]);
    props->port2 = nla_get_u32(tb[IFLA_HSR_PORT2]);
    if (tb[IFLA_HSR_MULTICAST_SPEC])
        props->multicast_spec = nla_get_u8(tb[IFLA_HSR_MULTICAST_SPEC]);
    if (tb[IFLA_HSR_SUPERVISION_ADDR])
        props->supervision_address = *nla_data_as(NMEtherAddr, tb[IFLA_HSR_SUPERVISION_ADDR]);
