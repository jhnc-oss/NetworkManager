...
_ip_nexthop_dump_parse_cb(const struct nl_msg *msg, void *arg)
{
    ...
    obj = _new_from_nl_nexthop(nlmsg_hdr(msg), FALSE);
    if (!obj)
        return NL_SKIP;

    if (data->addr_family != AF_UNSPEC) {
        if (NMP_OBJECT_GET_ADDR_FAMILY(obj) != data->addr_family)
            return NL_OK;
    }

    if (data->ifindex > 0) {
        if (obj->ip_nexthop.ifindex != data->ifindex)
            return NL_OK;
    }
...
_ip_nexthop_get_parse_cb(const struct nl_msg *msg, void *arg)
{
    ...
    data->obj = _new_from_nl_nexthop(nlmsg_hdr(msg), FALSE);
    if (!data->obj)
        return NL_SKIP;
