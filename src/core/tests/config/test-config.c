...
#define assert_config_value(config_data, group, key, expected_value) \
    _assert_config_value(config_data, group, key, expected_value, __FILE__, __LINE__)
...
test_config_simple(void)
{
    ...
    value = nm_config_data_get_value(nm_config_get_data_orig(config),
                                     "extra-section",
                                     "extra-key",
...
test_config_confdir(void)
{
    ...
    g_assert_cmpstr(nm_config_data_get_connectivity_uri(nm_config_get_data_orig(config)),
                    ==,
                    "http://example.net");
    g_assert_cmpint(nm_config_data_get_connectivity_interval(nm_config_get_data_orig(config)),
                    ==,
    ...
    value = nm_config_data_get_value(nm_config_get_data_orig(config),
                                     NM_CONFIG_KEYFILE_GROUPPREFIX_TEST_APPEND_STRINGLIST ".0",
                                     "new",
                                     NM_CONFIG_GET_VALUE_NONE);
    ...
    g_assert(!nm_config_data_has_value(nm_config_get_data_orig(config),
                                       "appendable-test",
                                       "non-appendable-key1",
...
#define ASSERT_GET_CONN_DEFAULT(xconfig, xname, xvalue)                             \
    ...
    value = nm_config_data_get_value(nm_config_get_data_orig(config),
                                     NM_CONFIG_KEYFILE_GROUPPREFIX_TEST_APPEND_STRINGLIST ".1",
                                     "val1",
                                     NM_CONFIG_GET_VALUE_NONE);
...
#define check_warning(str, group, key)                                                   \
    {                                                                                    \
        gs_free char *expected = NULL;                                                   \
                                                                                         \
        expected = g_strdup_printf("unknown key '%s' in section [%s] of file '" TEST_DIR \
                 ...
                 TestSetValuesCheckStateFcn check_state_fcn)
{
    ...
    if (check_state_fcn)
        check_state_fcn(config,
                        nm_config_get_data(config),
                        FALSE,
                        NM_CONFIG_CHANGE_NONE,
                        config_data_before);

    g_signal_handlers_disconnect_by_func(config,
                                         _set_values_config_changed_cb,
                                         &config_changed_data);
                                    ...
                                    NMConfigChangeFlags *out_expected_changes)
{
    g_key_file_set_string(keyfile,
                          NM_CONFIG_KEYFILE_GROUPPREFIX_INTERN "section1",
                          "key",
                                      ...
                                      NMConfigData       *old_data)
{
    g_assert(changes == NM_CONFIG_CHANGE_NONE);
    g_assert(
        !nm_config_data_has_group(config_data, NM_CONFIG_KEYFILE_GROUPPREFIX_INTERN "section1"));
}
                                  ...
                                  NMConfigData       *old_data)
{
    if (is_change_event)
        g_assert(changes
                 == (NM_CONFIG_CHANGE_CAUSE_SET_VALUES | NM_CONFIG_CHANGE_VALUES
                     | NM_CONFIG_CHANGE_VALUES_INTERN));
                                      ...
                                      NMConfigChangeFlags *out_expected_changes)
{
    ...
    *out_expected_changes =
        NM_CONFIG_CHANGE_VALUES | NM_CONFIG_CHANGE_VALUES_USER | NM_CONFIG_CHANGE_VALUES_INTERN;
}
                                        ...
                                        NMConfigChangeFlags *out_expected_changes)
{
    ...
    g_key_file_set_string(keyfile,
                          "atomic-prefix-1.section-a",
                          NM_CONFIG_KEYFILE_KEY_ATOMIC_SECTION_WAS,
                          "any-value");
    g_key_file_set_string(keyfile,
                          "non-atomic-prefix-1.section-a",
                          NM_CONFIG_KEYFILE_KEYPREFIX_WAS "nap1-key1",
                          "any-value");
    g_key_file_set_string(keyfile, "non-atomic-prefix-1.section-a", "nap1-key3", "intern-value3");
    g_key_file_set_string(keyfile,
...
test_config_set_values(void)
{
    ...
    _set_values_intern(config, _set_values_intern_internal_set, _set_values_intern_internal_check);
...
test_config_enable(void)
{
    ...
    config =
        setup_config(NULL, TEST_DIR "/NetworkManager.conf", "", NULL, TEST_DIR "/conf.d", "", NULL);
    assert_config_value(nm_config_get_data_orig(config),
