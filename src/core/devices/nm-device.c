                     ...
                     gboolean       require_all)
{
    ...
    /* Return early when the IP configuration is not ready */
    if (addr_bin->addr_family == AF_INET && priv->ip_data_4.state != NM_DEVICE_IP_STATE_READY) {
        return;
    }
    if (addr_bin->addr_family == AF_INET6 && priv->ip_data_6.state != NM_DEVICE_IP_STATE_READY) {
        return;
    }
...
nm_device_start_ip_check(NMDevice *self)
{
    ...
    if (gw_ping_timeout > 0 && ip_ping_timeout == 0) {
        ...
        _LOGD(LOGD_DEVICE,
              "starting ping on the gateway with a %u seconds timeout",
              gw_ping_timeout);
    ...
    if (ip_ping_timeout > 0) {
        ...
        strv              = nm_strvarray_get_strv_notempty(ip_ping_addresses, NULL);
