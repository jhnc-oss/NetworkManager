...
ip_check_ping_timeout_cb(gpointer user_data)
{
    ...
    _LOGW(LOGD_DEVICE, "ping: ping operations timed out");
...
nm_device_start_ip_check(NMDevice *self)
{
    ...
    ip_ping_timeout               = nm_setting_connection_get_ip_ping_addresses_timeout(s_con);
    priv->ping_operations         = NULL;
    ...
    if (gw_ping_timeout == 0 && ip_ping_timeout != 0) {
        ...
        _LOGI(LOGD_DEVICE, "started ping operations for ip-ping-addresses...");
