...
device_link_changed(gpointer user_data)
{
    ...
    if ((was_up && !priv->up) || (carrier_was_up && !priv->carrier)) {
         ...
         */
        controller = nm_device_get_master(self);
        if (controller && nm_device_get_device_type(controller) == NM_DEVICE_TYPE_BOND
            ...
            c_list_for_each_entry (info, &controller_priv->slaves, lst_slave) {
                if (info->slave != self && NM_DEVICE_GET_PRIVATE(info->slave)->carrier) {
                    _LOGT(LOGD_CORE,
                          "sending gARP on port %s (ifindex %d)",
                          nm_device_get_iface(info->slave),
                          nm_device_get_ifindex(info->slave));
                    if (nm_device_bond_announce_ports_on_slb(controller, info->slave))
                        break;
                }
            }
        }
