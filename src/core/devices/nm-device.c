...
cleanup_ping_operation(PingOperation *ping_op)
{
    ...
    if (ping_op->timeout) {
        nm_clear_g_source_inst(&ping_op->timeout);
    }
...
spawn_ping_for_operation(NMDevice *self, PingOperation *ping_op)
{
    ...
    if (ret) {
        ...
        _LOGW(ping_op->log_domain, "ping: could not spawn %s: %s", ping_op->binary, error->message);
    }
...
ip_check_ping_watch_cb(GPid pid, int status, gpointer user_data)
{
    ...
    if (WIFEXITED(status)) {
        if (WEXITSTATUS(status) == 0) {
            _LOGD(ping_op->log_domain, "ping: ping succeeded");
            success = TRUE;
        } else {
            _LOGW(ping_op->log_domain, "ping: ping failed with error code %d", WEXITSTATUS(status));
        }
    } else {
        _LOGW(ping_op->log_domain, "ping: stopped unexpectedly with status %d", status);
    }
...
ip_check_ping_timeout_cb(gpointer user_data)
{
    ...
    _LOGW(ping_op->log_domain, "ping: ping operation timed out");
...
static gboolean
start_ping(NMDevice *self, PingOperation *ping_op, guint timeout)
{
    NMDevicePrivate *priv = NM_DEVICE_GET_PRIVATE(self);

    if (spawn_ping_for_operation(self, ping_op)) {
        ping_op->timeout =
            nm_g_timeout_add_seconds_source(timeout, ip_check_ping_timeout_cb, ping_op);
...
nm_device_start_ip_check(NMDevice *self)
{
    ...
    if (buf[0]) {
        ...
        if (start_ping(self, ping_op, gw_ping_timeout))
            ping_started = TRUE;
    }

    if (ip_ping_timeout && !nm_g_array_len(ip_ping_addresses)) {
        const NML3ConfigData *l3cd;
        gs_strfreev char    **strv = nm_strvarray_get_strv_notempty_dup(ip_ping_addresses, NULL);
        guint                 i;
        for (i = 0; strv[i]; i++) {
            ...
            if (s[0]) {
                ...
                if (start_ping(self, ping_op, ip_ping_timeout))
                    ping_started = TRUE;
            }
        }
    }
