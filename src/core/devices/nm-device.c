...
typedef struct {
    ...
    union {
        struct {
            ...
            NMSettingIP4LinkLocal   enabled : 5;
        } v4;
        struct {
            NML3IPv6LL     *ipv6ll;
...
_prop_get_ipv4_link_local(NMDevice *self)
{
    ...
    /* DISABLED never sets any IPv4 LL address
     * ENABLED sets it at connection start and never unsets it
     * FALLBACK sets it when state changes if and only if it's not ready,
     * that is when there is no other address configured (either manually
     * set or obtained through DHCP) */
...
_dev_ipll4_check_fallback(NMDevice *self, const NML3ConfigData *l3cd_new)
{
    ...
    if (!l3cd_new || priv->ipll_data_4.v4.enabled != NM_SETTING_IP4_LL_FALLBACK) {
        return;
    }
...
activate_stage3_ip_config_for_addr_family(NMDevice *self, int addr_family, const char *method)
{
    ...
    if (IS_IPv4) {
        priv->ipll_data_4.v4.enabled = _prop_get_ipv4_link_local(self);
        if (priv->ipll_data_4.v4.enabled == NM_SETTING_IP4_LL_ENABLED)
            _dev_ipll4_start(self);
...
_cleanup_ip_pre(NMDevice *self, int addr_family, CleanupType cleanup_type, gboolean preserve_dhcp)
{
    ...
    if (IS_IPv4) {
        priv->ipll_data_4.v4.enabled = NM_SETTING_IP4_LL_DISABLED;
    } else {
