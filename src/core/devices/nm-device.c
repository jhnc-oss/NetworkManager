    ...
    if (nm_setting_ip_config_has_dhcp_request_options(s_ip)) {
        *options = g_malloc(nm_setting_ip_config_get_num_dhcp_request_options(s_ip));
        for (i = 0, j = 0; i < nm_setting_ip_config_get_num_dhcp_request_options(s_ip); i++) {
            ...
            val =
                _nm_utils_ascii_str_to_uint64(nm_setting_ip_config_get_dhcp_request_option(s_ip, i),
                                              10,
                                              0,
                                              256,
                                              256);
...
_dev_ipdhcpx_start(NMDevice *self, int addr_family)
{
    ...
    num_request_additional_options =
        _prop_get_ipvx_dhcp_request_options(self, addr_family, &request_additional_options);
