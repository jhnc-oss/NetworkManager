        ...
        if (ping_op->ping_addresses_require_all) {
            ...
            if (g_list_length(priv->ping_operations) == 0) {
                _LOGD(ping_op->log_domain,
                      ...
                      "succeeded");
                if (priv->timeout)
                    nm_clear_g_source_inst(&priv->timeout);
                ip_check_pre_up(self);
            }
        } else {
...
ip_check_ping_timeout_cb(gpointer user_data)
{
    ...
    _LOGD(ping_op->log_domain, "ping: ping operation timed out on %s", ping_op->address);

    cleanup_ping_operation(ping_op);
    priv->ping_operations = g_list_remove(priv->ping_operations, ping_op);

    if (g_list_length(priv->ping_operations) == 0) {
        _LOGW(ping_op->log_domain, "ping: all ping operations timed out");
        if (priv->timeout)
            nm_clear_g_source_inst(&priv->timeout);
        ip_check_pre_up(self);
    }
...
static gboolean
start_ping(NMDevice *self, PingOperation *ping_op, guint timeout, guint ping_addresses_len)
{
    NMDevicePrivate *priv = NM_DEVICE_GET_PRIVATE(self);

    if (spawn_ping_for_operation(self, ping_op)) {
        priv->ping_operations = g_list_append(priv->ping_operations, ping_op);
        if (g_list_length(priv->ping_operations) == ping_addresses_len) {
            priv->timeout =
...
nm_device_start_ip_check(NMDevice *self)
{
    ...
    buf[0]             = '\0';
    ping_addresses_len = nm_g_array_len(ip_ping_addresses);
    if (gw_ping_timeout && !ping_addresses_len) {
        const NMPObject      *gw;
        const NML3ConfigData *l3cd;
    ...
    if (buf[0]) {
        ...
        if (start_ping(self, ping_op, gw_ping_timeout, 1))
            ping_started = TRUE;
    }
    ...
    if (gw_ping_timeout && ping_addresses_len) {
        ...
        _LOGW(
            LOGD_DEVICE,
            "ping targets specified in 'connection.ip-ping-addresses' may delay reaching the "
        ...
        if (l3cd) {
            for (i = 0; strv[i]; i++) {
                ...
                if (s[0]) {
                    ...
                    if (start_ping(self, ping_op, gw_ping_timeout, ping_addresses_len))
                        ping_started = TRUE;
                }
            }
        }
