...
_check_complete(NMVpnConnection *self, gboolean success)
{
     ...
     * If this vpn connection does not have it's own device resort to nm_l3_config_data_new_from_connection
     * since we can't properly apply these properties anyway
     */
    if (ifindex > 0) {
        device = nm_manager_get_device_by_ifindex(NM_MANAGER_GET,
        ...
        l3cd = nm_l3_config_data_new_from_connection(nm_netns_get_multi_idx(priv->netns),
                                                     nm_vpn_connection_get_ip_ifindex(self, TRUE),
                                                     connection);
        _LOGD("VPN connection does not have it's own device. Some connection properties won't be "
              "supported.");
    }
