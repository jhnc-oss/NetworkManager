        ...
        if (sett_conn && old_state >= NM_DEVICE_STATE_PREPARE
            ...
            case NM_DEVICE_STATE_REASON_NO_SECRETS:
                ...
                if (con_v == 0 || con_v == nm_agent_manager_get_agent_version_id(priv->agent_mgr)) {
                    ...
                    s_ip4 = nm_device_get_applied_setting(device, NM_TYPE_SETTING_IP4_CONFIG);
                    s_ip6 = nm_device_get_applied_setting(device, NM_TYPE_SETTING_IP6_CONFIG);
                    if (s_ip4 && s_ip6) {
                        ip4_method = nm_setting_ip_config_get_method(s_ip4);
                        ip6_method = nm_setting_ip_config_get_method(s_ip6);
                        if (!nm_streq(ip4_method, NM_SETTING_IP4_CONFIG_METHOD_DISABLED)
                            && !nm_streq(ip6_method, NM_SETTING_IP6_CONFIG_METHOD_DISABLED)) {
                            _LOGD(LOGD_DEVICE, "block-autoconnect: IP4 and IP6 is disabled");
                            nm_settings_connection_autoconnect_blocked_reason_set(
                                sett_conn,
                                NM_SETTINGS_AUTOCONNECT_BLOCKED_REASON_NO_SECRETS,
                                TRUE);
