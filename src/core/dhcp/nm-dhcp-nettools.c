...
lease_to_ip4_config(NMDhcpNettools *self, NDhcp4ClientLease *lease, GError **error)
{
    ...
    for (i = 0; i < client_config->n_request_additional_options; i++) {
        r = _client_lease_query(lease, client_config->request_additional_options[i], &l_data, &l_data_len);
        if (r == 0) {
            v_str = nm_utils_bin2hexstr_fuller(l_data, l_data_len, '\0', TRUE, TRUE, NULL);
            _add_option(options, client_config->request_additional_options[i], v_str);
        }
    }
...
ip4_start(NMDhcpClient *client, GError **error)
{
    ...
    for(i = 0; i < client_config->n_request_additional_options; i++) {
        n_dhcp4_client_probe_config_request_option(config, client_config->request_additional_options[i]);
    }
