...
make_wireless_setting(shvarFile *ifcfg, GError **error)
{
    ...
    if (value) {
        chan = _nm_utils_ascii_str_to_int64(value, 10, 1, MAX_CHANNEL, 0);
        if (chan == 0) {
            g_set_error(error,
                        NM_SETTINGS_ERROR,
                        NM_SETTINGS_ERROR_INVALID_CONNECTION,
    ...
    if (value) {
        if (!NM_IN_STRSET(value, "6GHz", "a", "bg")) {
            g_set_error(error,
                        NM_SETTINGS_ERROR,
                        NM_SETTINGS_ERROR_INVALID_CONNECTION,
                        "Band '%s' invalid",
        ...
        /* remap backwards-compatible values */
        if (nm_streq(value, "a")) {
            g_free(value);
            value = g_strdup("5GHz");
        } else if (nm_streq(value, "bg")) {
            g_free(value);
            value = g_strdup("2.4GHz");
        }
        ...
        if (chan > NM_UTILS_MAX_CHAN_5GHZ) {
            g_set_error(error,
                        NM_SETTINGS_ERROR,
                        NM_SETTINGS_ERROR_INVALID_CONNECTION,
            ...
            g_free(value);
            goto error;
        } else if (chan > NM_UTILS_MAX_CHAN_2GHZ) {
            PARSE_WARNING(
                "Setting channel without band is ambiguous and deprecated. Assuming 5GHz.");
            g_object_set(s_wireless, NM_SETTING_WIRELESS_BAND, "5GHz", NULL);
        } else {
            PARSE_WARNING(
                "Setting channel without band is ambiguous and deprecated. Assuming 2.4GHz.");
