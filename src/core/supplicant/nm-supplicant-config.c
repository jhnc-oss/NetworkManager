...
/* The locally-administered bit is the second LSB of the first octet
 * (IEEE 802 individual/group addressing). */
#define ETH_ADDR_FIRST_OCTET 0
#define ETH_ADDR_LAA_BIT     0x02u
...
_seen_bssids_look_like_mlo_per_link(const char *const *seen_bssids, guint num_seen_bssids)
{
    ...
    for (i = 0; i < num_seen_bssids; i++) {
        ...
        if (!(addr[ETH_ADDR_FIRST_OCTET] & ETH_ADDR_LAA_BIT))
            return FALSE;
