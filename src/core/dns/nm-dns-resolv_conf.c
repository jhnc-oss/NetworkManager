...
typedef struct {
    ...
    gboolean   skip_connection_searches;
    gboolean   skip_connection_options;
...
merge_global_dns_config(NMResolvConfDataInProgress *rc, NMGlobalDnsConfig *global_conf)
{
    ...
    if (searches) {
        rc->skip_connection_searches = TRUE;
        for (i = 0; searches[i]; i++) {
            if (nm_dns_domain_is_routing(searches[i]))
                continue;
            if (!nm_dns_domain_is_valid(searches[i], FALSE, TRUE))
                continue;
            nm_dns_add_string_item(rc->searches, searches[i], TRUE);
        }
    }

    options = nm_global_dns_config_get_options(global_conf);
    if (options) {
        rc->skip_connection_options = TRUE;
        for (i = 0; options[i]; i++)
            nm_dns_add_string_item(rc->options, options[i], TRUE);
    }
    ...
    for (i = 0; servers[i]; i++) {
        ...
        nm_dns_add_string_item(rc->nameservers, addrstr, TRUE);
    }
               ...
               const NML3ConfigData       *l3cd)
{
    ...
    for (i = 0; i < num_nameservers; i++) {
        ...
        nm_dns_add_string_item(rc->nameservers, buf, TRUE);
    }

    if (!rc->skip_connection_searches) {
        nm_dns_add_domains(rc->searches, addr_family, l3cd, FALSE, TRUE);
    }

    has_trust_ad = FALSE;
    if (!rc->skip_connection_options) {
    ...
    if (addr_family == AF_INET) {
        ...
        for (i = 0; i < num; i++)
            nm_dns_add_string_item(rc->nis_servers, nm_inet4_ntop(nis_servers[i], buf), TRUE);
                          ...
                          const char        *hostdomain)
{
    ...
    if (hostdomain)
        nm_dns_add_string_item(data_in_progress.searches, hostdomain, TRUE);
