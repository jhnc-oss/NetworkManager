...
typedef struct {
    ...
    char                         *mode;
    /* Custom plugin is plugin handling configuration of
     * any advanced DNS resolution service */
    NMDnsPlugin *custom_plugin;
    /* sd_resolved has a special place and it is running even when
     * we have different plugin in place, to handle NM's own requests */
    NMDnsPlugin *sd_resolved_backup_plugin;
    NMDnsPlugin *resolvconf_plugin;
    /* hash computed from all the partial hashes*/
    guint8 full_hash[HASH_LEN];
...
static void
compute_full_hash(NMDnsManager *self, guint8 buffer[static HASH_LEN])
{
    NMDnsManagerPrivate             *priv = NM_DNS_MANAGER_GET_PRIVATE(self);
                             ...
                             gboolean              replace_all)
{
    ...
    if (!priv->updates_queue) {
        ...
        compute_full_hash(self, priv->full_hash);
...
nm_dns_manager_set_hostname(NMDnsManager *self, const char *hostname, gboolean skip_update)
{
    ...
    if (hostname && nm_utils_is_specific_hostname(hostname)
        ...
        if (domain) {
             ...
             */
            if (nm_dns_domain_is_valid(domain, TRUE, FALSE)) {
                /* pass */
            } else if (nm_dns_domain_is_valid(hostname, TRUE, FALSE)) {
                domain = hostname;
            }
...
_get_config_variant(NMDnsManager *self)
{
    ...
    c_list_for_each_entry (ip_data, head, ip_data_lst) {
        ...
        if (num > 0) {
            ...
            nm_dns_add_domains(array_domains, ip_data->addr_family, ip_data->l3cd, TRUE, FALSE);
