...
typedef struct {
    ...
    bool                          custom_plugin_update_to_do;
    NMDnsPlugin                  *sd_resolved_backup_plugin;
    bool                          sd_resolved_backup_plugin_update_to_do;
    NMDnsPlugin                  *resolvconf_plugin;
    bool                          resolvconf_plugin_update_to_do;
...
static gboolean
update_plugin_hashes(NMDnsManager *self)
{
    NMDnsManagerPrivate             *priv       = NM_DNS_MANAGER_GET_PRIVATE(self);
...
update_dns(NMDnsManager *self, gboolean no_caching, gboolean force_emit, GError **error)
{
    ...
    if (priv->custom_plugin_update_to_do && priv->custom_plugin)
...
nm_dns_manager_end_updates(NMDnsManager *self, const char *func)
{
    ...
    priv->config_changed = update_plugin_hashes(self);
...
again:
    ...
    if (plugin_changed) {
        /* We need to clear hash of resolvconf plugin to
         * trigger regeneration if needed */
        nm_dns_plugin_set_hash(priv->resolvconf_plugin, (guint8[HASH_LEN]) {0});
    }
