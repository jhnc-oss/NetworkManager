...
_set_fcn_ethtool(ARGS_SET_FCN)
{
    ...
    case NM_ETHTOOL_TYPE_FEC:
        if (string_list_to_fec_int(value, &fec_mode, error)) {
            nm_setting_option_set_uint32(setting, NM_ETHTOOL_OPTNAME_FEC, fec_mode);
            return TRUE;
        } else {
            return FALSE;
        }
...
static const NMMetaPropertyInfo *const property_infos_ETHTOOL[] = {
    ...
    PROPERTY_INFO (NM_ETHTOOL_OPTNAME_FEC,
                   ...
                   .property_typ_data =
                   DEFINE_PROPERTY_TYP_DATA_SUBTYPE
                      (ethtool, .ethtool_id = NM_ETHTOOL_ID_FEC)
                   ),
    NULL,
};
...
string_to_fec_int(const char *flag_str, uint32_t *fec_mode, GError **error)
{
    ...
    value_i64 = _nm_utils_ascii_str_to_int64(flag_str, 0, 0, UINT32_MAX, -1);
    if (value_i64 == -1) {
        nm_utils_error_set(error,
                           NM_UTILS_ERROR_INVALID_ARGUMENT,
                           _("'%s' is not valid; use any combination of "
                             "'auto', 'off', 'rs', 'baser', 'llrs' "
                             "or unsigned 32 bits integer"),
                           flag_str);
        *fec_mode = 0;
        return FALSE;
    }
