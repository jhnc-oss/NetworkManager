...
is_valid_addr(const char *addr)
{
    return (addr && strlen(addr) > 0 && !nm_streq(addr, "0.0.0.0") && !nm_streq(addr, "::")
            && nm_utils_ipaddr_valid(AF_UNSPEC, addr));
}
...
static char *
format_conn_name(const char *table_name, struct nbft_info_hfi *hfi, gboolean no_vlan)
{
    if (!no_vlan && hfi->tcp_info.vlan > 0)
...
static NMConnection *
create_wired_conn(struct nbft_info_hfi *hfi, const char *table_name, const char *hwaddr)
{
...
parse_hfi(GPtrArray *a, struct nbft_info_hfi *hfi, const char *table_name, char **hostname)
{
    ...
    case AF_INET:
        g_object_set(s_ip6,
                     ...
                     NM_SETTING_IP6_CONFIG_METHOD_DISABLED,
                     NULL);
        if (is_valid_addr(hfi->tcp_info.dhcp_server_ipaddr)) {
            g_object_set(s_ip4,
                         NM_SETTING_IP_CONFIG_METHOD,
