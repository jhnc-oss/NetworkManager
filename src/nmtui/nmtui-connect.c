...
on_rescan_cancel(NmtNewtForm *form, gpointer user_data)
{
    ...
    /* Close the rescan form if it's still open */
    if (nmt_newt_widget_get_realized(NMT_NEWT_WIDGET(batch_data->rescan_form))) {
        nmt_newt_form_quit(batch_data->rescan_form);
    }
    g_signal_handlers_disconnect_by_func(batch_data->rescan_form,
                                         G_CALLBACK(on_rescan_cancel),
                                         batch_data);
...
{
    gs_unref_object NmtNewtForm *rescan_form;
    NmtNewtWidget               *label;
...
on_rescan_complete(gpointer data)
{
    ...
    if (batch_data->has_error)
        nmt_newt_message_dialog(_("Wi-Fi scan failed for one or more devices"));

    /* We timed out so cancel any pending scans first */
    if (batch_data->timeout_occurred) {
        on_rescan_cancel(NULL, batch_data);
        nmt_newt_message_dialog(_("Wi-Fi scan timed out for one or more devices"));
    }
