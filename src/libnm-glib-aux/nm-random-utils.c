...
static ssize_t
getrandom_full(void *buf, size_t count, unsigned int flags)
{
    ssize_t ret;
    uint8_t *p = buf;
...
dev_random_wait(void)
{
    ...
    nm_assert(random_fd.fd >= 0 && poll(&random_fd, 1, -1) == 1);
    close(random_fd.fd);
    has_waited = true;
}

static ssize_t
urandom_read_full(void *buf, size_t count)
{
...
nm_random_get_bytes(void *p, size_t n)
{
    ...
    if (ret == 0)
        return;
    nm_assert(ret == 0 || errno == ENOSYS);

    dev_random_wait();
    ret = urandom_read_full(p, n);
    nm_assert(ret == 0);
}
