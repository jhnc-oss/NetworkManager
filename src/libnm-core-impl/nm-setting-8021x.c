...
{
    G_STATIC_ASSERT((NM_SETTING_802_1X_CK_FORMAT_UNKNOWN
                     == (NMSetting8021xCKFormat) NM_CRYPTO_FILE_FORMAT_UNKNOWN));
    G_STATIC_ASSERT(
        (NM_SETTING_802_1X_CK_FORMAT_X509 == (NMSetting8021xCKFormat) NM_CRYPTO_FILE_FORMAT_X509));
    G_STATIC_ASSERT((NM_SETTING_802_1X_CK_FORMAT_RAW_KEY
                     == (NMSetting8021xCKFormat) NM_CRYPTO_FILE_FORMAT_RAW_KEY));
    G_STATIC_ASSERT((NM_SETTING_802_1X_CK_FORMAT_PKCS12
                     == (NMSetting8021xCKFormat) NM_CRYPTO_FILE_FORMAT_PKCS12));

    nm_assert(NM_IN_SET(format,
               ...
               GError                **error)
{
    ...
    case PROP_PHASE2_CLIENT_CERT:
        if (value && scheme != NM_SETTING_802_1X_CK_SCHEME_PKCS11
            && !NM_IN_SET(format, NM_CRYPTO_FILE_FORMAT_X509, NM_CRYPTO_FILE_FORMAT_PKCS12)) {
            g_set_error_literal(error,
                                NM_CONNECTION_ERROR,
                                NM_CONNECTION_ERROR_INVALID_PROPERTY,
        ...
        p_cert = (property == PROP_CLIENT_CERT) ? &priv->client_cert : &priv->phase2_client_cert;
        break;
    ...
    if (p_password && !nm_streq0(password, *p_password)) {
        nm_free_secret(*p_password);
        *p_password = g_strdup(password);
    } else
        notify_password = PROP_0;

    nm_gobject_notify_together(setting, notify_cert, notify_password, notify_client_cert);
                                              ...
                                              const char     *phase2_altsubject_match)
{
    ...
    priv->phase2_altsubject_matches =
        g_slist_append(priv->phase2_altsubject_matches, g_strdup(phase2_altsubject_match));
                                                          ...
                                                          const char     *phase2_altsubject_match)
{
    ...
    for (iter = priv->phase2_altsubject_matches; iter; iter = g_slist_next(iter)) {
        if (!strcmp(phase2_altsubject_match, (char *) iter->data)) {
            priv->phase2_altsubject_matches =
                g_slist_delete_link(priv->phase2_altsubject_matches, iter);
            _notify(setting, PROP_PHASE2_ALTSUBJECT_MATCHES);
            return TRUE;
        }
    }
...
verify(NMSetting *setting, NMConnection *connection, GError **error)
{
    ...
    if (!NM_IN_STRSET(priv->phase2_auth,
                      NULL,
                      "pap",
                      "chap",
                      "mschap",
    ...
    if (!_cert_verify_property(priv->private_key, NM_SETTING_802_1X_PRIVATE_KEY, NULL, NULL, error))
        return FALSE;
