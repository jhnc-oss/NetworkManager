...
verify(NMSetting *setting, NMConnection *connection, GError **error)
{
    ...
    if (priv->shared_dhcp_lease_time && !nm_utils_validate_shared_dhcp_lease_time(priv->shared_dhcp_lease_time, error)) {
        g_prefix_error(error,
                       "%s.%s: ",
                       nm_setting_get_name(setting),
...
_nm_sett_info_property_override_create_array_ip_config(int addr_family)
{
    ...
    _nm_properties_override_gobj(
        ...
        &nm_sett_info_propert_type_direct_string,
        .direct_offset = NM_STRUCT_OFFSET_ENSURE_TYPE(char *, NMSettingIPConfigPrivate, shared_dhcp_range),
        .direct_string_allow_empty = TRUE);
