...
verify(NMSetting *setting, NMConnection *connection, GError **error)
{
    ...
    if (priv->channel_width != NM_SETTING_WIRELESS_CHANNEL_WIDTH_AUTO) {
        ...
        if (!nm_streq(priv->band, "6GHz") && !nm_streq(priv->band, "5GHz")
            && !nm_streq(priv->band, "a")) {
...
nm_setting_wireless_class_init(NMSettingWirelessClass *klass)
{
     ...
     * property: band
     * variable: BAND(+)
     * values: 6GHz, 5GHz, 2.4GHz, [deprecated] a, bg
     ...
     *   Channels greater than 14 mean "5GHz" or "6GHz" bands, otherwise the
     *   band is "2.4GHz".
     * example: CHANNEL=6
     * ---end---
     */
