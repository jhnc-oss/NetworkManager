...
NM_GOBJECT_PROPERTIES_DEFINE_BASE(PROP_PEER,
                                  ...
                                  PROP_WFD_GO_INTENT,
                                  PROP_WFD_PERSISTENT_RECONNECT,
                                  PROP_WFD_DEVICE_NAME,
...
typedef struct {
    ...
    gint32   wfd_go_intent;
    bool wfd_persistent_reconnect;
    char    *wfd_device_mode;
    char    *wfd_security_type;
    char    *wfd_device_name;

    /* Wi-Fi Display settings mostly for troubleshooting performance of p2p_find vs p2p_listen and the timing of calls on different platforms or hardware */
...
/**
 * nm_setting_wifi_p2p_get_wfd_go_intent
 * @setting: the #NMSettingWiFiP2P
 * 
 * Returns: the #NMSettingWifiP2P:wfd-go-intent property of the setting
 * 
 * since 1.36
 */
gint
nm_setting_wifi_p2p_get_wfd_go_intent(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);

    return NM_SETTING_WIFI_P2P_GET_PRIVATE(setting)->wfd_go_intent;
}

/**
 * nm_setting_wifi_p2p_get_wfd_persistent_reconnect
 * @setting: the #NMSettingWiFiP2P
 * 
 * Returns: the #NMSettingWifiP2P:wfd-persistent-reconnect property of the setting
 * 
 * since 1.36
 */
gboolean
nm_setting_wifi_p2p_get_wfd_persistent_reconnect(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);

    return NM_SETTING_WIFI_P2P_GET_PRIVATE(setting)->wfd_persistent_reconnect;
}

/**
 * nm_setting_wifi_p2p_get_wfd_device_name
 * @setting: the #NMSettingWiFiP2P
 * 
 * Returns: the #NMSettingWifiP2P:wfd-device-name property of the setting
 * 
 * since 1.36
 */
const char *
nm_setting_wifi_p2p_get_wfd_device_name(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);

    return NM_SETTING_WIFI_P2P_GET_PRIVATE(setting)->wfd_device_name;
}
 ...
 */
const char *
nm_setting_wifi_p2p_get_wfd_security(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);
...
gboolean
nm_setting_wifi_p2p_get_wfd_listen_only(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);
...
gint
nm_setting_wifi_p2p_get_wfd_listen_period(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);
...
gint
nm_setting_wifi_p2p_get_wfd_listen_interval(NMSettingWifiP2P *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_WIFI_P2P(setting), NULL);
...
verify(NMSetting *setting, NMConnection *connection, GError **error)
{
    ...
    if (priv->wfd_listen_interval <= 0 || priv->wfd_listen_period >= 600) {
        ...
        g_set_error(error,
                    NM_CONNECTION_ERROR,
                    NM_CONNECTION_ERROR_INVALID_PROPERTY,
                    _("'%i' is not a valid listen interval. Value must be greater or equal to "
                      "listen period"),
                    priv->wfd_listen_period);
        g_prefix_error(error,
                       "%s.%s: ",
...
nm_setting_wifi_p2p_class_init(NMSettingWifiP2PClass *setting_wifi_p2p_class)
{
    ...
    /**
     * NMSettingWifiP2P:wfd-persistent-reconnect:
     *
    ...
    _nm_setting_property_define_direct_boolean(properties_override,
                                               obj_properties,
                                               NM_SETTING_WIFI_P2P_WFD_PERSISTENT_RECONNECT,
                                               PROP_WFD_PERSISTENT_RECONNECT,
                                               FALSE,
                                               NM_SETTING_PARAM_FUZZY_IGNORE,
                                               NMSettingWifiP2P,
                                               _priv.wfd_persistent_reconnect);

    /**
     * NMSettingWifiP2P:wfd-device-name:
     *
