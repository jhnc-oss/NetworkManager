 ...
 * IPv4 addresses into an array of IP address strings. In case of error %NULL is
 * returned.
 *
...
{
    return _nm_utils_ip4_dns_from_variant(value, TRUE, NULL);
}
                                     ...
                                     GError  **error)
{
    ...
    if (!g_variant_is_of_type(value, G_VARIANT_TYPE("aau"))) {
        ...
        return g_ptr_array_ref(addresses);
    }
    ...
    for (i = 0; g_variant_iter_next(&iter, "@au", &item); i++) {
        ...
        if (labels && i < g_variant_n_children(labels)) {
            label = NULL;
 ...
 * ignored. In case of error %NULL is returned.
 *
 * Returns: (transfer full) (type utf8): a %NULL-terminated array of IP address strings.
 **/
char **
...
{
    return _nm_utils_ip6_dns_from_variant(value, TRUE, NULL);
}
