...
def add_host_sub(hostname: str, suffix: str = ""):
    ...
    if suffix == "" and re.search("\.({})$".format(domains), hostname):
        suffix = ".ext"
    ...
    for addr_str in ips4 + ips6:
        ...
        if addr.exploded not in ips_sub:
            if type(addr) is ipaddress.IPv4Address:
                addr_new = ipaddress.v4_int_to_packed(ip4_next)
                ip4_next += 1
            else:
                addr_new = ipaddress.v6_int_to_packed(ip6_next)
                ip6_next += 1
            ips_sub[addr.exploded] = ipaddress.ip_address(addr_new).compressed
...
hostnames.  Do not trust it and manually review that the log doesn't containsen
sitive data before sharing it.
